...
## 実装された改善点
### 0. ダッシュボード/マイカーのクイックウィン ✅

- ヘッダーブランド統一（`/public/icon.png` + サービス名）
- オンボーディング（はじめてガイド）バー
  - 3/3達成で自動非表示、右下に再表示チップ
  - 「閉じる」で `localStorage:onboardingHidden:v1:{uid}` に永続化
  - 未達に戻ったら自動再表示
- 軽量アラート（車検期限: 過期=赤、60日以内=黄）
- 給油表示統一（`getDisplayAmount()`/`getDisplayCost()`）
