...
**実装内容:**
- ヘッダーブランド統一（左上 `/public/icon.png` + サービス名）
- オンボーディング（はじめてガイド）バーの導入
  - 3/3達成で自動非表示、右下の再表示チップ
  - 「閉じる」選択時は `localStorage:onboardingHidden:v1:{uid}` に永続化
  - 未達に戻ったら自動再表示
- 軽量アラート（車検期限）
  - 過期: 赤バナー、60日以内: 黄バナー
  - CTAで関連セクションへ誘導
- 給油表示の統一（量/金額）
  - `getDisplayAmount()` と `getDisplayCost()` に統一
- 空状態の行動誘導
  - 「メンテナンスを追加」「給油を記録」「カスタマイズを追加」ボタン（モーダル起動 or マイカー遷移）
- 成功フィードバック
  - 給油/メンテ完了時に右下トースト（約1.8秒）
- 見出しの統一
  - `SectionHeader` コンポーネント導入（タイトル/サブタイトル/右側ボタン/サイズ sm, md）

**反映ファイル（抜粋）:**
- `src/app/dashboard/page.tsx`（オンボーディング、アラート、見出し、CTA、トースト）
- `src/components/mycar/MyCarPage.tsx`（軽量アラート、見出し統一）
- `SPECIFICATION.md`（仕様反映）
