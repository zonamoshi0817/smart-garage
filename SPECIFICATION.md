...
#### 3.5. 車両ヘルスインジケータ
**3色のステータスピル:**
- 🛢️ オイル交換: 残り○km / ○日（良好/警告/要注意）
- 🔧 ブレーキ&タイヤ: 残り○km
- 🔋 バッテリー: 交換後○ヶ月経過

**機能:**
- クリックで次回予定を1タップ追加
- 走行距離と経過日数から推定残量を自動計算

#### 3.6. 直近の活動タイムライン
**表示内容:**
- 過去30日の給油/メンテ/カスタム/保険を時系列表示
- 各行にアイコン、タイトル、説明、費用を表示
- 相対日付表示（今日、昨日、○日前、○週間前）
...
#### 3.7. コスト & 燃費ミニダッシュボード
**4枚のカード:**
1. **今月の維持費**: 燃料+メンテ+保険按分、スパークライン付き
2. **過去90日の平均燃費**: 満タン給油ベース
3. **今年の総コスト**: 前年比ピクト付き
4. **次の出費予測**: オイル交換等の予測費用
