...
### 10. マイカーページv2.0（新規実装）
#### 概要
グランツーリスモ風の詳細データ表示ページ。選択した車両のすべての情報を1ページで包括的に確認・管理できます。

#### レイアウト構成（上から順に）

##### 1. 車両ヘッダー（ミニヒーロー）
- **車両画像**: サムネイル表示、ホバーで「📷 画像を変更」CTA
- **基本情報**: 車名、年式、型式、現在ODO、直近メンテ日
- **ステータスバッジ**（状態に応じて色変更）:
  - 🔧 車検期限: あと○日（緑/黄/橙/赤）
  - 🛡️ 保険期限: あと○日（緑/黄/橙/赤）
  - ⭐ プレミアム: プレミアムユーザーのみ表示
...
- **メンテナンス統計**: 総回数、最終メンテナンス、総費用

**右パネル:**
- **給油統計**: 総回数、総給油量、総燃料費、燃費評価（S/A/B/C/D）
- **コストサマリー**: 総維持費、燃料費割合、メンテ費割合
- **車検情報**: 車検期限、残り日数（色分け表示）
- **パフォーマンス評価**: プログレスバーで可視化
  - 燃費効率（緑）
  - メンテナンス頻度（紫）
  - コスト効率（青）

**デザイン特徴:**
- ライトテーマ（white背景、カラフルなセクション）
- 等幅フォントで数値を整列表示
- セクション別カラーコード（青/緑/紫/橙/赤/黄）
- 視認性の高い文字サイズと太さ
...
**表示内容:**
- カスタム品: ブランド、型番、費用（部品代/工賃）、施工場所、メモ
- 純正品: 「純正」表示 + 登録促進UI
  - サブカテゴリの例を表示
  - 「+ カスタマイズを登録」ボタン
- 折りたたみ式（デフォルト閉じ）
...
##### 5. 車両ヘルスインジケータ
**3色のステータスピル:**
- 🛢️ オイル交換: 残り○km / ○日（avgKmPerMonthから推定）
- 🔧 ブレーキ&タイヤ: 最終実施距離からの推定残量
- 🔋 バッテリー: 最終交換からの経過月表示
...
##### 6. 直近の活動タイムライン
**過去30日の統合表示:**
- 給油/メンテ/カスタム/保険を時系列で表示
- 各行に「明細を見る」「同じ作業を複製」のクイックCTA
- 相対日付表示（今日、昨日、○日前）
- アイコンで種類を識別（⛽🔧✨🛡️）

##### 7. コスト & 燃費ミニダッシュボード
**4枚のカード:**
1. 今月の維持費（燃料＋メンテ＋保険の按分）+ スパークライン
2. 過去90日の平均燃費（満タン基準）
3. 今年の総コスト（前年比ピクト）
4. 次の出費予測（オイル○円、車検までの概算など）

**Rechartsによるミニスパークライン:**
- 過去6ヶ月のコストトレンドを1本表示
...
##### 9. メンテ & カスタムの次回提案カード
**自動提案項目:**
- オイル交換: あと1,300km → 「テンプレから作成」で推奨粘度/容量を自動セット
- オイルフィルター交換
- タイヤローテーション
- ブレーキフルード交換
- エアフィルター交換
...
##### 12. コンテキスト広告（無料ユーザーのみ）
- 車種・オイル粘度・タイヤサイズ連動の関連商品カード
- 事前高さ固定 + "🏷️ 広告"ラベル
- プレミアムユーザーは非表示
- 「広告を非表示にする →」リンクでアップグレード誘導

#### 技術実装
**コンポーネント構成:**
- `MyCarPage.tsx`: メインページ
- `VehicleHeader.tsx`: 車両ヘッダー
- `QuickActions.tsx`: クイックアクションボタン
- `VehicleSpecsPanel.tsx`: 車両データパネル
- `CustomPartsPanel.tsx`: カスタムパーツ一覧
- `VehicleHealthIndicator.tsx`: ヘルスインジケータ
- `ActivityTimeline.tsx`: 活動タイムライン
- `CostAndFuelDashboard.tsx`: コスト&燃費ダッシュボード
- `FuelAndPriceChart.tsx`: 燃費・単価チャート
- `NextMaintenanceSuggestion.tsx`: 次回提案
- `DocumentsAndDrafts.tsx`: ドキュメント管理
- `ShareAndPDF.tsx`: 共有&PDF
- `ContextualAd.tsx`: コンテキスト広告

**ユーティリティ:**
- `src/lib/dateUtils.ts`: Timestamp/Date変換ヘルパー
  - `toDate()`: Timestamp/Date → Date
  - `toMillis()`: Timestamp/Date → ミリ秒
  - `toTimestamp()`: Date → Timestamp
  - `daysFromNow()`: 今日からの日数

**プレミアム統合:**
- `usePremiumGuard`フックで機能制限
- ペイウォールモーダル（minimal variant）
- 無料/プレミアムで表示内容を切り替え

#### デザイン方針
- **レスポンシブ**: モバイル〜デスクトップ対応
- **カラーコード**: セクション別に色分け
- **視認性**: font-medium/bold、大きめの文字サイズ
